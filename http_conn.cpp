...
/* 网站的根目录 */
// const char* doc_root = "/home/mayichao/share/MYC_WebServer/root_file";
char* doc_root;
locker lock;
...
void addfd( int epollfd, int fd, bool one_shot = false ){
    ...
    /*
    在使用 ET 触发的时候，一个进程或线程在读取完某个 socket 上的数据后开始处理数据，
    而这个过程中该 socket 上又又新数据可读， EPOLLIN 再次触发，此时另一个线程
    被唤醒来处理这些新的数据。于是就出现了两个线程同时操作一个 socket 的局面。所以
    可以通过 epoll 的 EPOLLONESHOT 事件来实现
    对于注册了 EPOLLONESHOT 事件的文件描述符，操作系统最多触发其上注册的一个可读、可写
    或者异常事件，且只触发一次，除非我们使用 epoll_ctl 函数重置该文件描述符上注册的
    EPOLLONESHOT 事件。所以，当该 socket 被某个线程处理完毕，就应该立即重置 EPOLLONESHOT 事件
    */
    if( one_shot ){
...
void http_conn::init( int sockfd, const sockaddr_in& addr, char* root, string user, string passwd, string sqlname ){
    m_sockfd = sockfd;
    m_address = addr;
    /* 如下两行是为了避免 TIME_WAIT 状态，仅用于调试，实际使用时应该去掉 */
    // int reuse = 1;
    // /* 其实就是 socket重用来避免 TIME_WAIT */
...
void http_conn::init(){
    ...
    m_linger = false; /* HTTP 请求是否要求保持连接 */
...
http_conn::HTTP_CODE http_conn::parse_request_line( char* text ){
    ...
    *m_version++ = '\0';
    ...
    if( strncasecmp( m_url, "http://", 7 ) == 0 ){ //比较字符串前 n 个字符
        m_url += 7;
        m_url = strchr( m_url, '/' );
    }
...
/* 当得到一个完整、正确的 HTTP 请求时，我们就分析目标文件的属性。如果目标文件存在、对所有用户可读，且不是目录，则使用 mmap 将其
