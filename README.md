...
- 异步日志记录实现
  ...
  实现了简单的日志记录，了解了最基本的日志流程。

  本项目中日志由服务器自动创建，记录运行状态(以下是示例)

  > 2021-05-21 06:30:07.563543 [info]: adjust timer once

  > 2021-05-21 06:30:07.564553 [info]: adjust timer once

  > 2021-05-21 06:30:11.154982 [info]: adjust timer once

  > 2021-05-21 06:30:11.155439 [info]: adjust timer once

  实现的思想和原理：

  采用了生产者-消费者的思想，维护一个日志队列，用数组模拟了队列数据结构，考虑到当日志信息过大，如果同步写可能会造成线程阻塞，采用线程回调函数，后台异步地去处理(思考：这个可不可以注册到epoll中？)。(虽然本项目日志信息还是很小的，但是可以留一个习惯，习惯去写异步，多线程)

  依然是单例模式实现，因为C++11要求编译器保证内部静态变量的线程安全性，所以可以直接采用局部静态方法来实现单例模式，而不需要加锁
  ```cpp
  class Singleton{
  public:
